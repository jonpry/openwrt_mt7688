...
RUN sudo -iu openwrt bash -c "pwd; ls; ls .ssh/; cd openwrt; cp ./SGD-def.config .config; ./scripts/feeds update;./scripts/feeds install libpam libcares libgnutls libopenldap libidn libssh2 liblzma libnetsnmp; ./scripts/feeds install -a; make download"
